...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight=torch.nn.Parameter(torch.zeros(self.embed_size*self.n_features,self.hidden_size))
    ...
    def embedding_lookup(self, w):
        ...
        '''
        xij=torch.zeros(size=(len(w),self.n_features*self.embed_size))
        for i in range(len(w)):
            xij[i]=torch.index_select(self.embeddings,0,w[i]).view(self.n_features*self.embed_size)
        x=xij
        '''
        #print('embed',self.embeddings.shape)
        #print('w',w.shape)
        #xij=torch.gather(self.embeddings,1,w)
            #for j in range(len(w[0])):
                #xij[i][j]=self.embeddings[w[i][j]]
        #print('feature',self.n_features)
        #print('embed',self.embed_size)
        #print('be x', x.shape)
        #x=torch.reshape(xij,(len(w),self.n_features*self.embed_size))
    ...
    def forward(self, w):
        ...
        #print('x',x.shape)
        #print('wt',self.embed_to_hidden_weight.shape)
        #print('bias',self.embed_to_hidden_bias.shape)
