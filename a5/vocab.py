...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids=[]
        senlist=[]    #contains few sentence
        for sen in sents:
           wordlist=[]  #contains few words
           for word in sen:
              charlist=[]  #contains few chars
              charlist.append(self.start_of_word)
              for char in word:
                 intid=self.char2id[char]
                 charlist.append(intid)
              charlist.append(self.end_of_word)
              wordlist.append(charlist)
           senlist.append(wordlist)
        word_ids=senlist
        return word_ids
  
        #word2ind=words2charindices(sents)
        #word2maxword=pad_sents_char(word2ind,self.word2id['<pad>'])
        #print('pad',word2maxword.shape) 
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        word2ind=self.words2charindices(sents)
        word2indpadd=pad_sents_char(word2ind,self.word2id['<pad>'])
        #batch_size=len(word2indpadd)
        #max_sentence_length=len(word2indpadd[0])
        #max_word_length=len(word2indpadd[0][0])
        #print('shape',batch_size,max_sentence_length,max_word_length)
        #print(type(word2indpadd),type(word2indpadd[0]),type(word2indpadd[0][0]))
        #print('word2indpadd',len(word2indpadd),len(word2indpadd[0]),len(word2indpadd[0][0]))
        sents_var=torch.tensor(word2indpadd,dtype=torch.long, device=device)
        sents_var=torch.transpose(sents_var,0,1)
        #print('dtype',sents_var.dtype)
        #print('grad',sents_var.requires_grad)
        #print('shape',sents_var.shape)
        return sents_var
